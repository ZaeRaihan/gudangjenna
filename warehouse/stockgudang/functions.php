...
function tambahBarang($data) {
  ...
  // $tgl_brg_keluar = date('Y-m-d', strtotime($tgl_brg_keluar));
    ...
    // Query tambah barang
    $query = "INSERT INTO stockgudang (kode, collection, kategori, article_name, size, stock, harga, rak, lokasi, umur) VALUES 
...
function ubahBarang($data, $idstock)
{
    global $db;
   ...
   // $tgl_brg_keluar = date('Y-m-d', strtotime($tgl_brg_keluar));
    ...
    // Query ubah barang
    $query = "UPDATE stockgudang SET kode = '$kode', collection = '$collection', kategori = '$kategori', article_name = '$article_name', size = '$size', stock = '$stock', 
    harga = '$harga', rak = '$rak', lokasi = '$lokasi', umur = '$umur' WHERE idstock = $idstock";
...
function getBarangById($idstock) {
    global $db;
    $query = "SELECT * FROM stockgudang WHERE idstock = $idstock";
    $result = mysqli_query($db, $query);
...
function hapusBarang($id)
{
    ...
    // Query hapus barang
    $query = "DELETE FROM stockgudang WHERE idstock = $id";
    mysqli_query($db, $query);
    return mysqli_affected_rows($db);
}
...
function proses_copy($id) {
    ...
    $insertQuery = "INSERT INTO stockgudang (kode, collection, kategori, article_name, size, stock, harga, rak, lokasi, umur)
                    VALUES (
                        '{$dataToCopy['kode']}',
