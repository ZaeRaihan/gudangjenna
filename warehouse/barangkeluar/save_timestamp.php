...
if ($data) {
    ...
    foreach ($data->selectedRows as $idbarang_keluar) {
        // Simpan timestamp pada tabel barangkeluar untuk setiap idbarang_keluar
        $timestamp = date('Y-m-d H:i:s'); // Timestamp saat ini dalam zona waktu Indonesia Barat

        // Lakukan query untuk menyimpan timestamp pada tabel barangkeluar
        $query = "UPDATE barangkeluar SET print_timestamp = '$timestamp' WHERE idbarang_keluar = $idbarang_keluar";
