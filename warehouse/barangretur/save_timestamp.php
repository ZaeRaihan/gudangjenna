...
if ($data) {
    ...
    foreach ($data->selectedRows as $idbarang_retur) {
        // Simpan timestamp pada tabel barangretur untuk setiap idbarang_retur
        $timestamp = date('Y-m-d H:i:s'); // Timestamp saat ini dalam zona waktu Indonesia Barat

        // Lakukan query untuk menyimpan timestamp pada tabel barangretur
        $query = "UPDATE barangretur SET print_timestamp = '$timestamp' WHERE idbarang_retur = $idbarang_retur";
