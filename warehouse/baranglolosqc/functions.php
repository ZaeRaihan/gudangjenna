...
function tambahBarang($data)
{
    ...
    // Query tambah barang
    $query = "INSERT INTO barang_lolosqc (tgl_brg_masuk, launching_date, collection, article_name, size, stock, tgl_brg_keluar, sj_for_wh) VALUES ('$tgl_brg_masuk', '$launching_date', '$collection', '$article_name', '$size', '$stock', '$tgl_brg_keluar', '$sj_for_wh')";
...
function ubahBarang($data, $idbarang_lolosqc)
{
    global $db;
    ...
    // Query ubah barang
    $query = "UPDATE barang_lolosqc SET tgl_brg_masuk = '$tgl_brg_masuk', launching_date = '$launching_date', collection = '$collection', article_name = '$article_name', size = '$size', stock = '$stock', tgl_brg_keluar = '$tgl_brg_keluar', sj_for_wh = '$sj_for_wh' WHERE idbarang_lolosqc = $idbarang_lolosqc";
...
function getBarangById($idbarang_lolosqc)
{
    global $db;
    $query = "SELECT * FROM barang_lolosqc WHERE idbarang_lolosqc = $idbarang_lolosqc";
    $result = mysqli_query($db, $query);
...
function hapusBarang($id)
{
    ...
    // Query hapus barang
    $query = "DELETE FROM barang_lolosqc WHERE idbarang_lolosqc = $id";
    mysqli_query($db, $query);
    return mysqli_affected_rows($db);
}
...
function proses_copy($id)
{
    ...
    $insertQuery = "INSERT INTO barang_lolosqc (tgl_brg_masuk, launching_date, collection, article_name, size, stock, tgl_brg_keluar, sj_for_wh)
                    VALUES (
