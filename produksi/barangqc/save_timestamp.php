...
if ($data) {
    ...
    foreach ($data->selectedRows as $idbarang_qc) {
        // Simpan timestamp pada tabel barangqc untuk setiap idbarang_qc
        $timestamp = date('Y-m-d H:i:s'); // Timestamp saat ini dalam zona waktu Indonesia Barat

        // Lakukan query untuk menyimpan timestamp pada tabel barangqc
        $query = "UPDATE barangqc SET print_timestamp = '$timestamp' WHERE idbarang_qc = $idbarang_qc";
