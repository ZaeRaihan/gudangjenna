...
function tambahBarang($data)
{
    ...
    // Query tambah barang
    $query = "INSERT INTO barangqc (tgl_brg_masuk, launching_date, collection, article_name, size, stock, vendor, tgl_brg_keluar, sj_for_qc) VALUES ('$tgl_brg_masuk', '$launching_date', '$collection', '$article_name', '$size', '$stock', '$vendor', '$tgl_brg_keluar', '$sj_for_qc')";
...
function ubahBarang($data, $idbarang_qc)
{
    global $db;
    ...
    // Query ubah barang
    $query = "UPDATE barangqc SET tgl_brg_masuk = '$tgl_brg_masuk', launching_date = '$launching_date', collection = '$collection', article_name = '$article_name', size = '$size', stock = '$stock', vendor = '$vendor' , tgl_brg_keluar = '$tgl_brg_keluar', sj_for_qc = '$sj_for_qc' WHERE idbarang_qc = $idbarang_qc";
...
function getBarangById($idbarang_qc)
{
    global $db;
    $query = "SELECT * FROM barangqc WHERE idbarang_qc = $idbarang_qc";
    $result = mysqli_query($db, $query);
...
function hapusBarang($id)
{
    ...
    // Query hapus barang
    $query = "DELETE FROM barangqc WHERE idbarang_qc = $id";
    mysqli_query($db, $query);
    return mysqli_affected_rows($db);
}
...
function proses_copy($id)
{
    ...
    $insertQuery = "INSERT INTO barangqc (tgl_brg_masuk, launching_date, collection, article_name, size, stock, vendor , tgl_brg_keluar, sj_for_qc)
                    VALUES (
