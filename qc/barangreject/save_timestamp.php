...
if ($data) {
    ...
    foreach ($data->selectedRows as $idbarang_reject) {
        // Simpan timestamp pada tabel barangreject untuk setiap idbarang_reject
        $timestamp = date('Y-m-d H:i:s'); // Timestamp saat ini dalam zona waktu Indonesia Barat

        // Lakukan query untuk menyimpan timestamp pada tabel barangreject
        $query = "UPDATE barang_reject SET print_timestamp = '$timestamp' WHERE idbarang_reject = $idbarang_reject";
