...
$barangreject = query("SELECT * FROM barang_reject");

// Pagination
$limit = 20;
$page = isset($_GET['page']) ? $_GET['page'] : 1;
$totalRecords = count(query("SELECT * FROM barang_reject"));
...
$query = "SELECT * FROM barang_reject $date_filter $search_filter ORDER BY idbarang_reject DESC LIMIT $start, $limit";
$barangreject = query($query);

// Hitung jumlah total records untuk pencarian yang difilter dan rentang tanggal
$totalRecordsSearchDateFilter = count(query("SELECT * FROM barang_reject $date_filter $search_filter"));
$totalPagesSearchDateFilter = ceil($totalRecordsSearchDateFilter / $limit);

// Hitung jumlah total catatan untuk filtered date range
$totalRecordsDateFilter = count(query("SELECT * FROM barang_reject $date_filter"));
...
<body>
    ...
    <div class="content">
        <div class="container-fluid">
            <!------- HEADER ------->
            <h1 class="page-header">Inventori Pengeluaran Barang Reject
                <br><br><br>
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        ...
                        <form action="" method="GET" class="form-inline">
                            ...
                            <a href="barangreject.php" class="btn btn-warning mx-2">Clear</a>
                        </form>
                ...
                <div class="row" style="margin-top: 5px; margin-bottom: 5px">
                    <div class="col-md-10">
                        <form action="" method="GET" class="form-inline">
                            ...
                            <a href="barangreject.php" class="btn btn-warning mx-2">Clear Filter</a>
                        </form>
                    </div>
                    <div class="col-md-2 text-right">
                        <!-- "text-right" untuk menggeser teks ke kanan -->
                        <a href="barang_reject_history.php" class="btn btn-info mx-2" target="_blank">History</a>
                    </div>
                </div>
                ...
                <div class="table-responsive">
                    <table id="data-table" class="table table-hover">
                        ...
                        <?php foreach ($barangreject as $row) : ?>

                            <tr class="data-row">
                                <td><input type="checkbox" class="print-checkbox" data-id="<?= $row["idbarang_reject"]; ?>">
                                </td>
                                <td><?= $i; ?></td>
                                ...
                                <td>
                                    <div class="btn-group text-center" style="display: flex; justify-content: center;">
                                        <button type="button" class="btn btn-warning delete-button" data-id="<?= $row["idbarang_reject"]; ?>">Hapus</button>
                                        <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#ubahBarangModal<?= $row["idbarang_reject"]; ?>" data-id="<?= $row["idbarang_reject"]; ?>" data-page="<?= $page; ?>">Ubah</button>
                                        <button type="button" class="btn btn-success copy-button" data-id="<?= $row["idbarang_reject"]; ?>">Copy</button>
                                    </div>
                                </td>
                            </tr>

                            <div class="modal fade" id="ubahBarangModal<?= $row["idbarang_reject"]; ?>" tabindex="-1" role="dialog" aria-labelledby="ubahBarangModalLabel<?= $row["idbarang_reject"]; ?>" aria-hidden="true">
                                <div class="modal-dialog" role="document">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h3 class="modal-title" id="ubahBarangModalLabel<?= $row["idbarang_reject"]; ?>">
                                                Ubah Barang</h3>
                                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                <span aria-hidden="true">&times;</span>
                                            </button>
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <form action="proses_ubah.php" method="post" enctype="multipart/form-data">
                                                ...
                                                <input type="hidden" name="id" value="<?= $row["idbarang_reject"]; ?>">
                                                <div class="form-group">
                                                    <label for="tgl_brg_masuk">Tanggal Barang Masuk</label>
