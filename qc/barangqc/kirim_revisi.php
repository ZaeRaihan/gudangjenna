...
if (!empty($data_barangqc)) {
    ...
    if ($stok_dikirim <= $stock && $stok_dikirim > 0) {
        // Jika stok yang akan dikirim tidak melebihi stok yang ada pada barangqc dan jumlah stok yang dikirim lebih dari 0, lakukan pengiriman ke tabel barangrevisi
        $query_insert_barangrevisi = "INSERT INTO barangrevisi (tgl_brg_masuk, launching_date, collection, article_name, size, stock, vendor, tgl_brg_keluar, sj_for_produksi, idbarang_qc) 
                                  VALUES ('$tgl_brg_keluar', '$launching_date', '$collection', '$article_name', '$size', '$stok_dikirim', '$vendor', NOW(), '', $idbarang_qc)";
