...
if (!empty($data_barangqc)) {
    ...
    // Memeriksa apakah stok yang akan dikirim tidak melebihi stok yang ada
    if ($stok_dikirim <= $stock && $stok_dikirim > 0) {
        // Jika stok yang akan dikirim tidak melebihi stok yang ada dan jumlah stok yang dikirim lebih dari 0, lakukan pengiriman
        $query_insert_barang_lolosqc = "INSERT INTO barang_lolosqc (tgl_brg_masuk, launching_date, collection, article_name, size, stock, tgl_brg_keluar, sj_for_wh, idbarang_qc) 
                                    VALUES ('$tgl_brg_keluar', '$launching_date', '$collection', '$article_name', '$size', '$stok_dikirim', NOW(), '', $idbarang_qc)";

        if (mysqli_query($db, $query_insert_barang_lolosqc)) {
            // Data berhasil dikirimkan
            $pesan = "Data berhasil dikirimkan ke tabel barang_lolosqc.";
            ...
            $pesan = "Terjadi kesalahan saat memasukkan data ke tabel barang_lolosqc: " . mysqli_error($db);

            // Tampilkan pesan kesalahan menggunakan JavaScript
            echo "<script>alert('$pesan'); window.history.back();</script>";
        }
    } else {
        // Handle jika stok yang dikirim melebihi atau sama dengan stok yang ada, atau stok yang dikirim kurang dari 1
        $pesan = "Stok yang akan dikirim tidak valid atau melebihi stok yang ada.";
