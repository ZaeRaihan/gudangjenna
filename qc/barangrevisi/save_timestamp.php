...
if ($data) {
    ...
    foreach ($data->selectedRows as $idbarang_revisi) {
        // Simpan timestamp pada tabel barangrevisi untuk setiap idbarang_revisi
        $timestamp = date('Y-m-d H:i:s'); // Timestamp saat ini dalam zona waktu Indonesia Barat

        // Lakukan query untuk menyimpan timestamp pada tabel barangrevisi
        $query = "UPDATE barangrevisi SET print_timestamp = '$timestamp' WHERE idbarang_revisi = $idbarang_revisi";
